...
while (flag2) {
    ...
    if (userData.userName == userID && userData.userPass == userPass) {
        console.log(" ", " ", " ", " ", " ", " ", " ", " ", " ", chalk.cyan(`Welcome ${userData.userName}`));
        //Seconadry loop
        while (flag == true) {
                ...
                const interface_withdraw = await inquirer.prompt([
                    {
                        name: "myWithdraw",
                        type: "number",
                        message: chalk.greenBright("How much money you want to deposit ? (withdraw limit is 10,000) : "),
                    },
                ]);
                if (myBalance >= interface_withdraw.myWithdraw) {
                    if (interface_withdraw.myWithdraw <= 10000 &&
                        ...
                        console.log(chalk.yellowBright("---------------------------------"));
                        console.log(chalk.green(`Ammount withdraw: ${interface_withdraw.myWithdraw}`));
                    ...
                    console.log(chalk.bgRedBright("Sorry but you have insufficient balance."));
                }
            }
            //Fastcash
                ...
                const Fastcash = await inquirer.prompt([
                    {
                        name: "myFast",
                        type: "list",
                        message: chalk.greenBright("Select which of the amount you want to withdraw."),
                ...
                if (myBalance > 0) {
                    if (Fastcash.myFast == "1000") {
                        if (myBalance >= 1000) {
                            ...
                            console.log(chalk.yellowBright("------------------------------"));
                            console.log(chalk.yellowBright("", "", "", "", "", "Withdraw Succesful!"));
                            console.log(chalk.yellowBright("------------------------------"));
                            console.log(chalk.magenta(`Your remaining balance is: ${myBalance}`));
                            console.log("");
                        }
                            ...
                            console.log("");
                            console.log(chalk.redBright("Withdraw failed due to insufficient balance !"));
                            console.log("");
                        }
                    }
                        ...
                        if (myBalance >= 5000) {
                            ...
                            console.log("");
                            console.log(chalk.redBright("Withdraw failes due to insufficient balance!"));
                        }
                    }
                        ...
                        if (myBalance >= 7000) {
                            ...
                            console.log(chalk.redBright("Withdraw failed due to insuffient balance!"));
                        }
                    }
                    ...
                    console.log(chalk.redBright(chalk.italic("Sorry but you have inssuficient balance")));
                }
            }
            //CheckBalance
            if (selectedFunction.myFunction == chalk.magentaBright("CheckBalance")) {
                console.log(chalk.yellowBright("-------------------------------------------------"));
                console.log(chalk.yellowBright(``, ``, ``, ``, `Account Holder: ${userID}`));
                console.log(chalk.yellowBright(``, ``, ``, `Card number: XXXX-XXXX-XXXX-XXX`));
                console.log(chalk.yellowBright(``, ``, ``, `Account created: 10/3/2004`));
                console.log(chalk.bold(chalk.yellow(``, ``, ``, `Your curent balance is : ${myBalance}`)));
                console.log(chalk.yellowBright("-------------------------------------------------------"));
                console.log("");
            }
            // Transfer
            if (selectedFunction.myFunction == chalk.blueBright("Transfer")) {
                const transfer = await inquirer.prompt([
                    {
                        name: "name",
                        type: "string",
                        message: chalk.whiteBright("Enter the person's username whom you want to transfer money:"),
                    },
                    {
                        name: "money",
                        type: "number",
                        message: chalk.cyan("Enter amount you want to transfer (You can only transfer money within 30000):"),
                    },
                ]);
                if (transfer.money > 0 && transfer.money <= 30000) {
                    if (myBalance >= transfer.money) {
                        ...
                        console.log(chalk.blueBright(`Amount received by : ${transfer.name}`));
                        console.log("-------------------------------------------------------------");
                        console.log("");
                        console.log(chalk.bold(chalk.green(`Amount transferred: ${transfer.money}`)));
            ...
            if (selectedFunction.myFunction ==
                ...
                if (options.myOption == chalk.grey("Sindh Bank")) {
                    console.log("");
                    console.log(chalk.yellowBright("Sorry but the service of this bank is not avalaible right now !"));
                    console.log("");
                    console.log("");
                }
                else if (options.myOption == chalk.blueBright("United Bank Limited") ||
                    chalk.green("Habib Bank Limited") ||
                    chalk.magentaBright("Meezan Bank") ||
                    chalk.red("Bank-Alfala") ||
                    chalk.greenBright("Bank-Al-Habib")) {
        ...
        count = count + 1;
        console.log(chalk.redBright("Invalid username or security pin. Please try again"));
        if (count == 3) {
            console.log("");
